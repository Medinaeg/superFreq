...
performPreliminaryVariantCallingOnMissingVCFs = function(metaData, reference, cpus=1) {
  ...
  systemCalls = paste0('/anaconda3/bin/samtools mpileup -d 1000 -q 15 -Q 15 -A -f ', reference, ' ',  metaData$BAM[missing], ' | varscan mpileup2cns - --variants --strand-filter 0 --p-value 0.01 --min-var-freq 0.02 --output-vcf 1 > ', metaData$VCF[missing])
